...
export const PageTile = (
  { id, filename, url, title, size, tags, is_media }: Page,
) => html`
  <article class="article">
    <header>
      <a
        href="/${is_media ? 'media' : 'archive'}/${filename}" class="title"
      >
        ${title}
      </a>
    </header>
    ...
    <div class="info tags" style="display: ${tags.length > 0 ? 'flex' : 'none'};">
